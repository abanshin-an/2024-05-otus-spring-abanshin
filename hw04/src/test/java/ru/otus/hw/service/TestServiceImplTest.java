...
class TestServiceImplTest {
    ...
    void executeTestFor() {
        ...
        doReturn("").when(ansiFormatter).format(any(),anyString());

        testService.executeTestFor(student);

        verify(ioService, times(2)).readIntForRangeWithPrompt(eq(1), eq(3), anyString(), anyString());
        verify(ioService, times(1)).printLine(eq(expectedQuestions.get(0).text()));
        verify(ioService, times(1)).printLine(eq(expectedQuestions.get(1).text()));
